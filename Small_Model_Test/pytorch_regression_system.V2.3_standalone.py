...
class EnhancedSmallDatasetRegressor:
    ...
    def __init__(self, test_size=0.2, validation_split=0.2, early_stop_patience=30,
                 use_combined_augmentation=False, n_augmentations=1):
        """
    ...
    def save_report(self, filename="report.md", save_path=None):
    ...
    def load_data_from_upload(self, file_path=None):
    ...
    def prepare_regression_data(self, data, target_columns=None, feature_columns=None):
    ...
    def design_architectures(self, input_dim, output_dim, data_size):
        ...
        # Deep FFN
        hidden_dims = [min(128, data_size * 2), min(64, data_size), min(32, data_size // 2)]
        architectures['Deep_FFN'] = lambda: DeepFFN(input_dim, output_dim, hidden_dims)
        
        # Wide Network
        hidden_dims = [min(256, data_size * 4), min(128, data_size * 2)]
        ...
        self.log(f"\n🏗️ Designed {len(architectures)} architectures")
    ...
    def train_and_evaluate_models(self, X, y):
        ...
        for aug_name, (X_aug, y_aug) in augmented_datasets.items():
            ...
            if self.use_combined_augmentation and aug_name == 'combined_augmented':
                ...
                X_aug_temp, _, y_aug_temp, _ = train_test_split(
                    X_aug, y_aug, test_size=self.test_size, random_state=42
                )
                ...
                try:
                    ...
                    for arch_name, arch_builder in architectures.items():
                        ...
                        try:
                            ...
                            self.log(f"  {arch_name}: MAE={mae:.4f}, R²={r2:.4f}, MAPE(ori)={mape_mean:.1f}%, Epoch={best_epoch}/{stopped_epoch}")
                            ...
                            self.log(f"  ✗ {arch_name}: {str(e)[:100]}")
                
                except Exception as e:
                    self.log(f"  ✗ Scaler {scaler_name}: {str(e)[:100]}")
        ...
        if results:
            ...
            self.log(f"  R²: {results[best_model_name]['r2']:.4f}")
            ...
            self.log(f"  R²: {results[best_model_byMAPE]['r2']:.4f}")
    ...
    def _plot_augmentation_analysis(self):
        ...
        try:
            ...
            self.log(f"⚠️ Error in augmentation analysis: {e}")
    ...
    def print_results_table(self):
        ...
        self.log(f"  R²: {self.results[self.best_model_name]['r2']:.4f}")
        ...
        self.log(f"  R²: {self.results[self.best_model_byMAPE]['r2']:.4f}")
    ...
    def save_model(self, filenames=[], folder_path=None):
        ...
        for model_name in filenames:
            ...
            try:
                ...
                # 為每個模型建立使用說明
                self._create_usage_instructions(filename, model_package)
    ...
    def save_augmented_datasets(self, folder_path=None):
    ...
    def _create_usage_instructions(self, filename, model_package):
...
- **Performance**:
  ...
  - R²: {model_package['performance']['r2']:.4f}
...
def run_comprehensive_regression_analysis():
    ...
    try:
        ...
        regressor.log(f"⚠️ Plot error: {e}")
...
def run_cli_mode():
    ...
    regressor.log("\\n📂 STEP 1: Loading Data")
    if args.file:
        data, filename = regressor.load_data_from_upload(file_path=args.file)
    elif args.simulate:
    ...
    try:
        X, y = regressor.prepare_regression_data(data, target_columns=target_cols, feature_columns=feature_cols)
    except Exception as e:
    ...
    if not args.no_save_report:
        regressor.log("\\n💾 STEP 5: Saving Report")
        regressor.save_report(save_path=args.output_dir)
    ...
    if not args.no_save_model:
        ...
        if models_to_save:
            regressor.save_model(filenames=models_to_save, folder_path=args.output_dir)
            
    # Save Augmented Data
    if args.save_data:
        regressor.log("\\n💾 STEP 7: Saving Augmented Datasets")
        regressor.save_augmented_datasets(folder_path=args.output_dir)
